...
// Circularidade: 4*π*área/perímetro².  1.0 = círculo perfeito.
// Maçã medida no vídeo: circ~0.23-0.43  → rejeitada abaixo de REJEITAR.
// Laranja: circ~0.55-0.78              → aceite acima de MINIMA.
// Entre os dois limiares → blob IRREGULAR (mostrado mas assinalado).
#define CIRCULARIDADE_REJEITAR 0.50   // abaixo disto: ignorado completamente
#define CIRCULARIDADE_MIN      0.65   // abaixo disto (mas >= REJEITAR): IRREGULAR
...
int main(void) {
    ...
    while (key != 'q') {
        ...
        vc_hsv_segmentation(image_hsv, image_seg,
                            ...
                            120, 255,  // Smin: excluir fundo; maca filtrada pela circularidade
                            60, 255);  // Vmin, Vmax
        ...
        if (blobs != NULL && nblobs > 0) {
            ...
            // ---------------------------------------------------
            for (int i = 0; i < (int)tracker.size(); i++)
                if ((int)tracker[i].rastro.size() >= 2)
            ...
            for (int i = 0; i < nblobs; i++) {
                ...
                double circ      = calcular_circularidade(blobs[i].area,
                                                          blobs[i].perimeter);
                // Rejeitar completamente blobs com forma muito nao-circular
                // (maca medida: circ~0.23-0.43, abaixo de CIRCULARIDADE_REJEITAR)
                if (circ < CIRCULARIDADE_REJEITAR) continue;
