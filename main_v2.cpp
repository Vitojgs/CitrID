...
int main(void) {
    ...
    while (capture.read(frame)) {
        ...
        //   S >= 150: exclui zonas pouco saturadas (brilho, fundo)
        ...
        // CLOSE kernel=15: fecha buracos (reflexos, pedunculo)
        ...
        if (blobs != NULL && nlabels > 0) {
            ...
            for (int i = 0; i < nlabels; i++) {
                ...
                // --- Filtro 2: ratio e compacidade da bounding box ---
                //
                // Problema: a maca vermelha e a maca verde passam o filtro HSV
                // porque tem pixels com cor proxima da laranja.
                // Solucao: dois filtros geometricos combinados:
                //
                // RATIO (largura/altura normalizado para [0,1]):
                //   Laranjas sao redondas -> ratio ~1.0
                //   Fragmentos da maca vermelha sao alongados -> ratio ~0.2
                //   Threshold: rejeitar se ratio < 0.65
                //
                // COMPACIDADE (area real / area da bounding box):
                //   Laranjas preenchem bem a bbox -> compacidade ~0.75
                //   Maca verde apanhada so na borda -> compacidade ~0.27
                //   Threshold: rejeitar se compacidade < 0.55
                //
                // EXCECAO: blobs na borda do frame sao sempre aceites
                // (sao laranjas parcialmente fora do campo de visao)
                ...
                // Informacao de debug (area e perimetro)
                // A Pessoa 3 substitui isto pelo calibre e categoria
